...
==== Rails 3.1+ and Java Container

The assets files are included at the root of the generated +war+ file and thus not in the +public+ folder.
On Rails 3.1+ you then need to include the +manifest+ files so they can be detected and used by the Asset Pipeline.
Use the +config.includes+ option in your +warble.rb+ file :
