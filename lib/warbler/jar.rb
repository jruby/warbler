...
module Warbler
  ...
  class Jar
    ...
    def run_javac(config, compiled_ruby_files)
      ...
      version_flag = RUBY_VERSION =~ /^1\.9/ ? '--1.9' : ''
      %x{java -classpath #{config.java_libs.join(File::PATH_SEPARATOR)} org.jruby.Main #{version_flag} -S jrubyc \"#{compiled_ruby_files.join('" "')}\"}
    end
